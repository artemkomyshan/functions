...
template <typename Func> class shared_function
{
public:
  explicit operator bool () const;

  template <typename... Args> std::result_of_t<Func (Args...)> operator() (Args... args) const;
  ...
  template <typename OFunc1, typename = Requires<Not<std::is_same<typename std::decay_t<OFunc1>, shared_function> > >,
...
shared_function<Func>
make_shared_function (Func f)
{
  return shared_function<Func> (std::move (f));
}

template <typename Func> shared_function<Func>::shared_function (std::nullptr_t) : shared_function () {}

template <typename Func> auto shared_function<Func>::operator= (std::nullptr_t) -> shared_function &
{
  mCall = nullptr;
  return *this;
}
...
shared_function<Func>::shared_function (OFunc1 &&call) : mCall (std::make_shared<Func> (std::forward<OFunc1> (call)))
{
}
...
template <typename OFunc1, typename, typename>
shared_function<Func> &
shared_function<Func>::operator= (OFunc1 &&call)
{
  mCall = std::make_shared<Func> (std::forward<OFunc1> (call));
  return *this;
}

template <typename Func> shared_function<Func>::operator bool () const { return mCall && mCall.get (); }
...
template <typename Func>
void
shared_function<Func>::swap (shared_function &other)
{
...
template <typename Func>
const Func &
shared_function<Func>::get () const
{
  return *mCall;
}
...
template <typename Func>
inline bool
operator== (const shared_function<Func> &func, std::nullptr_t) noexcept
{
...
template <typename Func>
inline bool
operator== (std::nullptr_t, const shared_function<Func> &func) noexcept
{
  return !static_cast<bool> (func);
}
...
template <typename Func>
inline void
swap (shared_function<Func> &l, shared_function<Func> &r)
{
  l.swap (r);
}
